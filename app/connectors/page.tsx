...
import { ConnectorsClient, type ConnectorListItem } from "@/components/connectors/ConnectorsClient";
...
export default async function ConnectorsPage() {
  ...
  const { data: connectors } = await supabase
    .from("connectors")
    .select("id, name, company, role, interviewed, can_introduce_customers")
    ...
    <div className="min-h-dvh bg-gray-50 pb-24">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base font-bold text-gray-900">{t("connectors.title")}</h1>
        </div>
      </header>

      <main className="mx-auto max-w-2xl space-y-3 p-3">
        <ConnectorsClient connectors={(connectors ?? []) as ConnectorListItem[]} />
      </main>
